...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy[:, 1] = np.int_(
        np.floor((lidar_pcl_cpy[:, 1] - configs.lim_y[0]) / bev_discretization))
    ...
    ''' 
    intensity_map[np.int_(lidar_top_pcl[:, 0]), np.int_(
        lidar_top_pcl[:, 1])] = lidar_top_pcl[:, 3] / np.max(lidar_top_pcl[:, 3])  # normalize intensity
    '''
    #아래 코드는 chatgpt의 조언
    #intensity_map[np.int_(lidar_top_pcl[:, 0]), np.int_(lidar_top_pcl[:, 1])] = lidar_top_pcl[:, 3] / (np.percentile(lidar_top_pcl[:, 3], 99) - np.percentile(lidar_top_pcl[:, 3], 1))

    #흑백이미지를 만들기 위한 chatgpt의 조언
    ...
    ####### ID_S2_EX2 END ####### 
# 여기 아래에 추가합니다. --> 이미지의 사각셀에 이미지 강도를 숫자로 표시 (chatgpt도움)
img_intensity = (intensity_map * 256).astype(np.uint8)  # 이미지를 8비트로 변환

# 새 창으로 intensity map 출력 (숫자 표시 포함)
def display_intensity_map_with_numbers(intensity_map):
        import matplotlib.pyplot as plt

        fig, ax = plt.subplots()
        cax = ax.matshow(intensity_map, cmap='gray')  # intensity_map을 그레이스케일로 출력
        plt.colorbar(cax)

        # 각 셀에 숫자를 출력
        for i in range(intensity_map.shape[0]):
            for j in range(intensity_map.shape[1]):
                c = int(intensity_map[i, j])
                if c != 0:  # 값이 0이 아닌 경우에만 표시
                    ax.text(j, i, str(c), va='center', ha='center', color='red')

        # 창에 이미지 표시
        plt.show()

        # intensity map과 숫자화된 맵을 새 창에 표시
        cv2.imshow("Intensity map", img_intensity)  # 흑백 intensity 맵
        display_intensity_map_with_numbers(intensity_map)  # 숫자 포함 맵을 새 창에 출력
        cv2.waitKey(0)
    ...
    height_map[np.int_(lidar_top_pcl[:, 0]), np.int_(lidar_top_pcl[:, 1])] = lidar_top_pcl[:, 2] / (
        configs.lim_z[1] - configs.lim_z[0])  # normalize height
    '''
    #chatgpt조언
