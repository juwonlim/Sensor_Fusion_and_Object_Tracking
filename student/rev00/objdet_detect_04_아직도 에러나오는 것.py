...
def load_configs_model(model_name='darknet', configs=None):
    ...
    #모델 이름에 따라 다른 모델 로드   
        ...
        configs.K = 50  # 추가됨
        configs.peak_thresh = 0.2  # 추가됨
        configs.save_test_output = False  # 추가됨
        configs.output_format = 'image'  # 추가됨
        configs.output_video_fn = 'out_fpn_resnet_18'  # 추가됨
        configs.input_size = (608, 608)  # 추가됨
        configs.hm_size = (152, 152)  # 추가됨
        configs.down_ratio = 4  # 추가됨
        configs.max_objects = 50  # 추가됨
        configs.num_center_offset = 2  # 추가됨
        configs.num_z = 1  # 추가됨
        configs.num_dim = 3  # 추가됨
        configs.num_direction = 2  # 추가됨 (sin, cos)
        configs.num_input_features = 4  # 추가됨
...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        outputs = model(input_bev_maps) #모델의 추론을 담는 변수, 이 딕셔너리는 보통 모델의 출력값을 저장, 이 outputs는 모델이 예측한 여러 값을 딕셔너리 형태로 반환하는데, 여기에는 hm_cen, cen_offset, direction, z_coor, dim 같은 키가 포함되어 있어야 함 
                                        #여기서 모델의 출력이 Tensor임(chatgpt의 조언) 그때문에 evaluation_utils.py파일에 post_processing함수가 numpy배열처리 함수이기에 결국 값을 이 파일내에서 변환해야함.
        ...
        if 'darknet' in configs.arch:
            ...
            # 여기에서 Tensor를 numpy로 변환 --> 하기의 주석부문 오류 예방위함임
            '''
            File "Loop_over_dataset_3-1.py", line 210, in <module>
            detections det detect objects(lidar bev, model det, configs_det) File "/workspace/home/student/objdet detect.py", line 275, in detect objects
            output post post File "/workspace/home/tools/objdet models/resnet/utils/evaluation utils.py", line 133, in post processing
            processing(decoded output, configs) #수정후--> 다음의 에러방지목적 TypeError: post processing() got an unexpected keyword argument conf thresh")
            get yawidetections[i, inds, 7:9]).astype (np.float32)], axis=1) AttributeError: Tensor object has no attribute astype
            '''
            decoded_output = decoded_output.cpu().numpy()  #여기에서 Tensor를 numpy로 변환 (evaluation_utils.py파일에 post_processing함수가 numpy배열처리하는 함수이기에)

            # post-processing 함수에 전달
            output_post = post_processing(decoded_output, configs) #evaluation_utils.py파일에 post_processing함수에 전달
            
            # post_processing에서 반환된 결과가 Tensor이면 Numpy로 변환
            for i in range(len(output_post)):
                if isinstance(output_post[i], torch.Tensor):
                    output_post[i] = output_post[i].cpu().numpy()  # Numpy로 변환
